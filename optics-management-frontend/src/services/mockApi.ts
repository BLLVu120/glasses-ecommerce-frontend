  ...
  async handle(config: AxiosRequestConfig, options: MockOptions = {}): Promise<AxiosResponse<any>> {
    ...
    try {
      ...
      if (method === 'get' && url === '/users/me') {
        // Profile endpoint - feature deleted
        return buildAxiosResponse(config, { code: 0, result: {} } as any);
      }
      ...
      if (method === 'get' && url.startsWith('/management/orders')) {
        // Manager feature - deleted
        return buildAxiosResponse(config, ok({ items: [], page: 0, size: 10, totalElements: 0, totalPages: 0 }));
      }
      ...
      if (method === 'get' && url === '/orders/me') {
        // Orders endpoint - related to deleted profile feature
        return buildAxiosResponse(config, ok({ items: [], page: 0, size: 10, totalElements: 0, totalPages: 0 }));
      }
      ...
      if (method === 'post' && url === '/payment/checkout') {
        ...
        if (typeof orderIdParam === 'string') {
          ...
          const paymentUrl = `https://sandbox.vnpayment.vn/mock-checkout?orderId=${encodeURIComponent(orderId)}`;
      ...
      if (url.startsWith('/production/orders')) {
        // Operation-staff feature deleted
        if (method === 'put' && /\/production\/orders\/.+\/start$/.test(url)) return buildAxiosResponse(config, ok({}));
        if (method === 'put' && /\/production\/orders\/.+\/finish$/.test(url)) return buildAxiosResponse(config, ok({}));
      ...
      if (url.startsWith('/ship/orders')) {
        ...
        if (method === 'get' && url === '/ship/orders/my-orders-accepted') return buildAxiosResponse(config, ok({ items: [] }));
      }

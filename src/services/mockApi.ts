  ...
  async handle(config: AxiosRequestConfig, options: MockOptions = {}): Promise<AxiosResponse<any>> {
    ...
    try {
      ...
      if (method === 'get' && url === '/users/me') {
        const baseUser = mockUsers.find((u) => u.role === 'CUSTOMER') ?? mockUsers[0];
        const profile: UserProfile = {
          ...
          dob: baseUser.dob,
          imageUrl: baseUser.imageUrl ?? null,
          email: baseUser.email,
          phone: baseUser.phone,
          roles: [
            {
              name: baseUser.role,
              description: baseUser.role,
              permissions: [],
            },
          ],
        };
        return buildAxiosResponse(config, { code: 0, result: profile } as any);
      }
